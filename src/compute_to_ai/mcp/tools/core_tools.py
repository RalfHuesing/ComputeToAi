...
def _load_plan(working_directory: Path, plan_name: str) -> Plan:
    ...
    if not plan_file.exists():
        raise ValueError(f"no plan named {plan_name!r}; create it first with create_plan")
...
def register_core_tools(mcp: FastMCP, working_directory: Path) -> None:
    """Register the Milestone 1 core tools, see Docs/10-Roadmap.md Epic 1.4."""

    @mcp.tool()
    def create_plan(plan_name: str, step_count: int) -> str:
        ...
        logger.info("create_plan: plan=%r status=ok", plan_name)
        logger.debug("create_plan args: step_count=%d", step_count)
        return f"created plan {plan_name!r} with {step_count} steps"

    @mcp.tool()
    def add_store(plan_name: str, store_name: str, initial_balance: float = 0.0) -> str:
        ...
        _save_plan(working_directory, plan)
        logger.info("add_store: plan=%r store=%r status=ok", plan_name, store_name)
        logger.debug("add_store args: initial_balance=%s", initial_balance)
        return f"added store {store_name!r} to plan {plan_name!r}"

    @mcp.tool()
    def add_effect(plan_name: str, store_name: str, amount_per_step: float) -> str:
        ...
        _save_plan(working_directory, plan)
        logger.info("add_effect: plan=%r store=%r status=ok", plan_name, store_name)
        logger.debug("add_effect args: amount_per_step=%s", amount_per_step)
        return f"added effect on store {store_name!r} to plan {plan_name!r}"

    @mcp.tool()
    def run_plan_simulation(plan_name: str) -> str:
        """Run the Milestone 1 deterministic simulation for a Plan."""
        plan = _load_plan(working_directory, plan_name)
        ...
        logger.info("run_plan_simulation: plan=%r status=ok", plan_name)
        logger.debug("run_plan_simulation result: %s", result.final_balances)
        return f"simulation for plan {plan_name!r} complete"

    @mcp.tool()
    def get_result(plan_name: str, include_time_series: bool = False) -> dict[str, Any]:
        """Return the final balances (and optionally the time series) of a Plan's last run."""
        result_file = _result_file(working_directory, plan_name)
        if not result_file.exists():
            raise ValueError(f"no result for plan {plan_name!r}; call run_plan_simulation first")
        result = SimulationResult.model_validate_json(result_file.read_text(encoding="utf-8"))
        logger.info("get_result: plan=%r status=ok", plan_name)
        payload = (
            result.model_dump()
            if include_time_series
            else {"final_balances": result.final_balances}
        )
        logger.debug("get_result payload: %s", payload)
        return payload
