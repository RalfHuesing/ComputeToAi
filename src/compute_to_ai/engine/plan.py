...
class Plan(BaseModel):
    ...
    (migration question, see Docs/08-Offene-Fragen.md).
    """
    ...
    effects: list[FixedEffect] = Field(default_factory=list)

    def store(self, name: str) -> Store:
        for store in self.stores:
            if store.name == name:
                return store
        raise KeyError(f"no store named {name!r} in plan {self.name!r}")
