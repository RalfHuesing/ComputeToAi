...
2. **`compute_to_ai.features.*`** (z. B. `finance`, `calculations`) bauen auf `compute_to_ai.engine` auf (z. B. registrieren sie eigene `Component`-Bausteine) und sind **untereinander unabhängig** – `finance` importiert nichts aus `calculations` und umgekehrt. Ein neues Feature-Modul lässt sich ergänzen, ohne bestehende Feature-Module anzufassen. Jeder Baustein (registrierter `ComputedEffect`) bekommt ein eigenes Pydantic-Modell für sein `parameters`-Dict; die schreibende `add_*`-Funktion und die lesende Baustein-Funktion validieren beide über dasselbe Modell statt Dict-Keys als Zeichenketten zu matchen – ein Tippfehler wird so zum Typfehler statt zu einem still ignorierten Default.
