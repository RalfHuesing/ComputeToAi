...
- **Kern-Tools**: Plan anlegen/kopieren/umbenennen/löschen, Speicher/Effekt hinzufügen, auflisten, gezielt entfernen (`core_list_stores`/`core_list_effects`/`core_remove_effect`), Simulation starten, Ergebnis abfragen, einen Pfad-Audit für wenige repräsentative Läufe mit vollem Ledger erzeugen (`core_run_path_audit`, siehe 01-Kern-Domaenenmodell.md, „Ledger"). „Ändern" ist bewusst kein implizites Upsert-by-Name beim erneuten Hinzufügen – zwei absichtlich gleichnamige Effekte sollen nicht überraschend kollidieren –, sondern entfernen und neu hinzufügen, gestützt auf die Auflistungs-Tools.
- **Feature-Tools je aktiviertem Modul**: z. B. Finanzen-Tools (Haushalt/Person anlegen, Einkommensstrom und Ausgaben mit Frequenz- und Intervall-Parametern wie `"monthly"`, `"yearly"`, `"every_n_years"` hinzufügen, zentrale Parameter-Registry verwalten via `finance_set_plan_parameter` und `finance_get_plan_parameters`, Baustein „Korrelierte Anlageklassen" aktivieren, Kategorie-Zeitreihe bzw. Event-Log eines Pfad-Audits abfragen, siehe 04-Feature-Finanzen-Methodik.md, „Pfad-Audit und Plausibilitätsprüfung"), Berechnungen-Tools (siehe 06-Feature-Berechnungen.md).
...
**Rückgabewert-Konvention – strukturiertes Echo statt Bestätigungs-String**: Jedes mutierende Baustein-Tool (`*_add_*`, `core_add_effect`, `core_add_transfer`) gibt die tatsächlich gespeicherten Werte als strukturiertes Objekt zurück, nicht nur eine Erfolgsmeldung. Zurückgegeben werden gezielt die Werte, die im Tool berechnet oder umgerechnet werden (z. B. der frequenz-gefaltete `amount_per_step`, der Rentenbetrag nach Abschlag/Zuschlag, der normalisierte Transferbetrag) – nicht ein Voll-Dump des Effekts. So kann der Agent seine eigene Eingabe unmittelbar gegen das prüfen, was der Server daraus gemacht hat, ohne einen zusätzlichen List-Aufruf.
...
Abgrenzung: Dieses Prinzip gilt für Größenordnungs-/Fachwissens-Urteile über ein Ergebnis, die Weltwissen und Einschätzung erfordern. Rein strukturelle/logische Konsistenz einer Plan-Konfiguration und ihrer tatsächlichen Ausführung – eindeutig entscheidbar, ohne Fachwissen oder Schwellenwert-Heuristik – prüft dagegen `finance_audit_plan` als granulares, komponierbares Tool (siehe 04-Feature-Finanzen-Methodik.md, „Pfad-Audit und Plausibilitätsprüfung"); auch dessen Ergebnis bleibt eine Liste roher Hinweise, die der Agent einordnet und dem Nutzer erklärt, keine automatische Entscheidung des Servers.
