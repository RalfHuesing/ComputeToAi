...
Der Zeitstrahl definiert Startzeitpunkt, Dauer und Schrittweite eines Simulationslaufs. Beides ist frei wählbar – eine Finanzsimulation läuft typischerweise über 40–80 Jahre in Jahresschritten, ein anderer Anwendungsfall könnte über Stunden in Minutenschritten laufen. Der Kern selbst trifft keine Annahme über sinnvolle Schrittweiten oder Einheiten; das ist Sache des jeweiligen Feature-Moduls bzw. Plans. Das Feld `Timeline.steps_per_year` macht diese Wahl explizit (Default 12, also Monatsschritte) – Feature-Module, die einen kalenderbasierten Rhythmus (z. B. `frequency: "monthly"/"yearly"` bei periodischen Cashflows, siehe 04-Feature-Finanzen-Methodik.md) in Zeitschritte umrechnen müssen, rechnen relativ zu diesem Wert statt eine feste Schrittweite anzunehmen. Das gilt über periodische Cashflows hinaus für alle Bausteine, die Alter oder Kalenderjahre in Schritte übersetzen: den Lebensphasen-Aufbau (Altersgrenzen → Phasengrenzen), die Rentenabschlag/-zuschlag-Berechnung (Schrittabstand → Monate) und den Entnahmepuffer des Cash-Buckets (Pro-Schritt-Ausgaben → Jahresausgaben).
...
**Namens-Referenzen werden zur Konfigurationszeit validiert**: Sowohl Phasennamen (`active_phases` eines Effekts) als auch Speichernamen, die ein Baustein referenziert, müssen beim Anlegen auf bereits registrierte Phasen bzw. Speicher zeigen, sonst schlägt der Aufruf sofort mit einer Fehlermeldung fehl, die alle unbekannten Namen nennt (`Plan.validate_active_phases` bzw. `Plan.validate_store_names`). Ohne diese Prüfung würde ein Tippfehler stillschweigend akzeptiert und der referenzierende Effekt bliebe in der Simulation einfach wirkungslos, statt zur Konfigurationszeit zu scheitern. Ausgenommen sind Speichernamen, die den vom Baustein selbst neu angelegten, „eigenen" Speicher bezeichnen (z. B. der Verbindlichkeits-Speicher einer Verbindlichkeit) – diese werden bei Nichtvorhandensein automatisch angelegt, siehe 03-Feature-Finanzen-Domaenenmodell.md.
...
Optional, pro Simulationslauf einschaltbar: Bei einem instrumentierten Lauf hält der Kern zusätzlich zum Zeitverlauf der Salden ein Ledger – für jeden Zeitschritt, welcher Effekt (nach Name bzw. Funktionsname) den Saldo welches Speichers um welchen Betrag verändert hat. Der Kern selbst ordnet diesen Änderungen keine fachliche Bedeutung zu (kein „Einkommen", keine „Steuer") – das ist Sache des jeweiligen Feature-Moduls (siehe 04-Feature-Finanzen-Methodik.md für die Finanz-Interpretation). Ergänzend hält ein instrumentierter Lauf den nach Laufende verbliebenen Zustand des Parameter-Dicts jedes berechneten Effekts fest (z. B. einen einmaligen Auslöse-Zeitpunkt), da dieser Laufzeitzustand sonst mit der laufinternen Kopie der Effekte verworfen würde.

Aus Kostengründen wird das Ledger nicht für jeden Monte-Carlo-Lauf geführt, sondern nur gezielt für wenige repräsentative Pfade nachträglich erzeugt (ein Pfad-Audit): Zunächst läuft eine normale Monte-Carlo-Simulation, dann wird je gewünschtem Perzentil (typischerweise Median und ein ungünstiges Perzentil) der Lauf-Index ermittelt, dessen Endsaldo (Summe über die Zielbedingungs-Speicher oder eine andere angegebene Speichermenge) diesem Perzentil am nächsten kommt – genau dieser Lauf wird anschließend erneut ausgeführt, diesmal instrumentiert. Da die korrelierten Renditeziehungen pro Lauf-Index deterministisch reproduzierbar sind (gleicher Seed, gleiche Lauf-Anzahl, siehe „Korrelation" oben), liefert der erneute Lauf exakt denselben Pfad. Ergänzt um den deterministischen Referenzlauf (der ohnehin nur ein einzelner Lauf ist) ergibt das die Grundlage für eine nachträgliche Plausibilitätsprüfung einzelner Pfade.
