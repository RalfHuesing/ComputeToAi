...
- **Verkaufspriorität**: Zwischen verschiedenen Positionen (unterschiedliche ISINs/Konten) besteht echte Wahlfreiheit, welche zuerst verkauft wird – anders als **innerhalb** einer einzelnen Position, wo die Verbrauchsfolge der Lots depotrechtlich fix nach FIFO läuft (siehe „Besteuerung" unten). Bei einer Entnahme wird deshalb zunächst unter den Positionen ohne Bestandsschutz-Vorteil diejenige mit dem geringsten unrealisierten Gewinn (in Prozent) ausgewählt, erst wenn diese erschöpft ist die mit dem nächsthöheren Gewinn; Bestandsschutz-Positionen werden unabhängig von ihrem aktuellen Gewinn so lange wie möglich geschont, da ihr Wert nicht im heutigen Steuervorteil liegt, sondern in der dauerhaften Steuerfreiheit aller künftigen Wertsteigerungen.
...
**Referenzierte Speicher müssen vorab existieren**: Bausteine unterscheiden zwei Arten von Speicher-Parametern. Ein Speicher, den der Baustein selbst neu anlegt und der ihm „gehört" (der Verbindlichkeits-Speicher einer Verbindlichkeit, der Positions-Speicher einer Anlageklasse), wird bei Nichtvorhandensein automatisch angelegt. Ein Speicher, den der Baustein lediglich *referenziert* – der Cash-Speicher von Cash-Bucket, Verbindlichkeit und Steuer-Manager, der Zielspeicher von Einkommen/Ausgabe/Anschaffung/Rente sowie die Portfolio-Gewichts- und Anlageklassen-Schlüssel –, muss dagegen bereits im Plan existieren; sonst schlägt der Aufruf zur Konfigurationszeit fehl (siehe 01-Kern-Domaenenmodell.md, „Namens-Referenzen werden zur Konfigurationszeit validiert"). Ein Tippfehler erzeugt so einen sofortigen Fehler statt eines leeren Phantom-Speichers oder eines wirkungslosen Effekts.
